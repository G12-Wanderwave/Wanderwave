...
constructor(
   ...
   */
  suspend fun downloadAndDisplayImage(imageUrl: String, imageView: ImageView) {
      val imageData =
      ...
      imageData?.let {
          Log.d("SpotifyController", "Image data downloaded successfully, size: ${it.size} bytes")
          val bitmap = BitmapFactory.decodeByteArray(it, 0, it.size)
        imageView.setImageBitmap(bitmap)
      }
        Log.d("SpotifyController", "outside the let, size")
   ...
   */
  // Using getAlbumImage in order to download and display image in an ImageView
  suspend fun displayAlbumImage(albumId: String, imageView: ImageView) {
      Log.d("SpotifyController", "beginning")
      getAlbumImage(albumId).collect { imageUrl ->
          Log.d("SpotifyController", "beginning2")
