...
class LocationUpdatesServiceTest {
    ...
    fun test1() = runBlocking {
        ...
        every { fusedLocationProviderClient.requestLocationUpdates(any(), capture(locationCallbackSlot), any()) } returns mockk()
        ...
        every { ActivityCompat.checkSelfPermission(context, Manifest.permission.ACCESS_COARSE_LOCATION) } returns PackageManager.PERMISSION_GRANTED
        ...
        every { anyConstructed<FusedLocationProviderClient>().requestLocationUpdates(any(), capture(locationCallbackSlot), any()) } returns mockk()
