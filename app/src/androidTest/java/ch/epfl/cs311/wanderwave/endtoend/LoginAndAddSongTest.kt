...
class LoginAndAddSongTest {
  ...
  fun endToEndLoginAddSong() = runBlockingTest {
    ...
    onComposeScreen<ProfileScreen>(composeTestRule) {
      assertIsDisplayed()
      addTrackButton.assertIsDisplayed()
      addTrackButton.performClick()
    }

    val expectedListItem = ListItem("id", "title", null, "title1", "subtitle1", false, true)
    val expectedListItem2 = ListItem("id", "title", null, "title2", "subtitle2", false, false)

    every { spotifyController.getAllElementFromSpotify() } returns flowOf(listOf(expectedListItem))
    every { spotifyController.getAllChildren(any()) } returns flowOf(listOf(expectedListItem2))

    mockProfileViewModel.retrieveAndAddSubsection()
    mockProfileViewModel.retrieveChild(expectedListItem)
    advanceUntilIdle() // Ensure all coroutines are completed

    onComposeScreen<SelectSongScreen>(composeTestRule) {
      assertIsDisplayed()
